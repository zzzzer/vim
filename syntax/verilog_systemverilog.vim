...
if exists("verilog_no_bracket_error")
  syn keyword verilogBlock       class endclass
else
  " Try to mark {}, [], (), module/endmodule and begin/end mismatches
  " (also case|casex|casez/endcase, function/endfunction, task/endtask
  syn region verilogClassMatch matchgroup=Statement start="\<class\>" end="\<endclass\>" contains=ALLBUT,verilogClassError,verilogCommentSpecial
  syn match  verilogClassError "\<endclasse\>"
endif
...
" By: Jimmie
""""""""""""""""""""""""""""""
"syn region myClassFold start=+\<class\>+ skip=+//+ end=+\<endclass\>+ keepend transparent fold
syn region myClassFold start=+\<class\>+ skip=+//+ end=+\<endclass\>+ matchgroup=verilogClassMatch keepend transparent fold
syn region myMethodFold start=+function\|task+ skip=+//+ end=+endfunction\|endtask+ keepend extend transparent fold
