...
"syn keyword vhdlArcCon contained architecture
syn keyword vhdlStatement array attribute
"syn keyword vhdlStatement begin block body buffer bus
"syn keyword vhdlBlockBegin contained begin
syn keyword vhdlStatement block body buffer bus

"syn keyword vhdlStatement case component configuration constant
"syn keyword vhdlStatement case configuration constant
syn keyword vhdlStatement configuration constant
syn keyword vhdlStatement disconnect downto
"syn keyword vhdlStatement elsif end entity exit
"syn keyword vhdlBlockEnd contained end
syn keyword vhdlStatement elsif entity exit

syn keyword vhdlStatement file for function
"syn keyword vhdlStatement generate generic group guarded
syn keyword vhdlStatement generate group guarded
...
syn keyword vhdlStatement package postponed procedure pure
...
"contains=ALLBUT,vhdlComment
"syn match   vhdlArc "architecture.*is"
"contains=vhdlArcCon
...
"syn match   vhdlArraySlice contained      "\<\(.*downto.\{-}\)\>"
"syn region vhdlArcMatch matchgroup=Error start="\<architecture.*is\_\s*begin\>" end="\<end architecture\>" keepend extend transparent fold
"syn region vhdlArcMatch matchgroup=Error start="\<architecture.*is\>" end="\<end architecture\>" keepend extend transparent fold
syn region vhdlArcMatch matchgroup=Statement start="\<component\>" end="\<end component\>" keepend extend transparent fold
syn region vhdlProcMatch matchgroup=Statement start="\<process\>" end="\<end process\>" keepend extend transparent fold
syn region vhdlProcMatch matchgroup=Statement start="\(end\)\@<! if" end="\<end if\>" keepend extend transparent fold
syn region vhdlProcMatch matchgroup=Statement start="\(end\)\@<! case" end="\<end case\>" keepend extend transparent fold
"syn region vhdlProcMatch matchgroup=Statement start="\<case \(.*\)\@<= is\>" end="\<end case\>" keepend extend transparent fold
syn region vhdlProcMatch2 matchgroup=Statement start="\<port map\>" end=";" contained transparent fold
syn region vhdlProcMatch3 matchgroup=Statement start="\<generic map\>" end=";" contains=vhdlProcMatch2 keepend extend transparent fold
"syn region vhdlProcMatch2 matchgroup=Error start=".*:\_.*\(port\|generic\) map" end=";" contained keepend extend transparent fold
"contains=vhdlArraySlice
"syn region vhdlProcMatch matchgroup=Error start="\<.*:.*\(\n.*\)\{0,3}\(generic map\_.*\)\?port map\>" end="\<);\>" keepend extend transparent fold
"syn region myMethodFold start="begin" skip="--" end="end" keepend extend transparent fold
"contains=vhdlBlockBegin,vhdlBlockEnd
